...
def get_logo_image():
    logo_img = pyglet.resource.image(get_random_letter()+'.png')
...
def on_key_press(symbol, modifiers):
    if symbol == pyglet.window.key.P:
        pyglet.image.get_buffer_manager().get_color_buffer().save(get_random_letter()+'.png')
...
def spawn_logo(dt):
    ...
    shape.friction = 0.5
    shape.letter = get_random_letter()
    body.position = x, y
    body.angle = angle

    space.add(body, shape)

    sprite = pyglet.sprite.Sprite(get_logo_image(), batch=batch)
    sprite.shape = shape
    sprite.body = body
    logos.append(sprite)
